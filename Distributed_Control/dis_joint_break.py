...
if __name__ == '__main__':
    ...
    # # ===== 途中再開用：既存結果の読み込み =====
    # re_results_file_path = r"C:\Users\rsait\OneDrive\ドキュメント\git\M1_styudy\result\2025\12\29\033614\aa.txt"  # 既存の結果ファイルパス
    ...
    start_trial_index = 0  # 途中再開用のインデックスを0に設定（初期状態）
    ...
    for i in tqdm(range(start_trial_index, num_trials),desc="Simulating Trials", unit="trial"):
        ...
        while (random_goal == initial_position or np.sqrt(random_goal[0]**2 + random_goal[1]**2) < 0.4 * L * joint_num or np.sqrt(random_goal[0]**2 + random_goal[1]**2) > L * joint_num):
            random_goal = [np.random.randint(0,100), np.random.randint(0, 100)]
        if first == 1:
            first = 0
