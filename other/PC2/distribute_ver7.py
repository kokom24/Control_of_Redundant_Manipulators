...
from multiprocessing import Manager, Pool, Process, Queue, Pipe, cpu_count
...
def joint_main(i, tmax, dt, L, theta, plt_x, plt_y, target_pos_x, target_pos_y, joint_n, ran, k, d, shm_name, shm_data):
    # Set parameters
    t = 0 
    re_x_list = []  
    re_y_list = []  
    ...
    # Set parameters
    theta_negative = shm_data[6 * (joint_n - 1)]  # Get the angle from shared memory
    theta_self = shm_data[6 * joint_n]  # Get the angle of the current joint from shared memory
    pos_negative = [shm_data[6 * (joint_n - 1) + 1], shm_data[6 * (joint_n - 1) + 2]] if joint_n > 0 else [0, 0]  # Get the position of the previous joint from shared memory
    v_positive = [shm_data[6 * (joint_n + 1) + 3], shm_data[6 * (joint_n + 1) + 4]] if joint_n < len(shm_data) // 6 - 1 else [0, 0]  # Get the velocity of the next joint from shared memory
    pos_positive = [shm_data[6 * (joint_n + 1) + 1], shm_data[6 * (joint_n + 1) + 2]]
    # print("plt_x", plt_x)
    # print("plt_y", plt_y)
    v_negative = [shm_data[6 * (joint_n - 1) + 3], shm_data[6 * (joint_n - 1) + 4]] if joint_n > 0 else [0, 0]  # Get the velocity of the previous joint from shared memory
    ...
    while t < tmax:
        ...
        if joint_n == 1:
            theta_negative = shm_data[6 * (joint_n - 1)] 
            theta_positive = shm_data[6 * (joint_n + 1)]
            v_negative = [shm_data[6 * (joint_n - 1) + 3], shm_data[6 * (joint_n - 1) + 4]] 
            v_positive = [shm_data[6 * (joint_n + 1) + 3], shm_data[6 * (joint_n + 1) + 4]] 
            pos_negative = [shm_data[6 * (joint_n - 1) + 1], shm_data[6 * (joint_n - 1) + 2]] 
            pos_positive = [shm_data[6 * (joint_n + 1) + 1], shm_data[6 * (joint_n + 1) + 2]] 
            status= [shm_data[joint_n * 6 + 5] for joint_n in range(len(shm_data) // 6)]
        else:
            theta_negative = shm_data[6 * (joint_n - 2)] 
            theta_positive = shm_data[6 * (joint_n)]
            v_negative = [shm_data[6 * (joint_n - 2) + 3], shm_data[6 * (joint_n - 2) + 4]] 
            v_positive = [shm_data[6 * (joint_n) + 3], shm_data[6 * (joint_n) + 4]] 
            pos_negative = [shm_data[6 * (joint_n - 2) + 1], shm_data[6 * (joint_n - 2) + 2]] 
            pos_positive = [shm_data[6 * (joint_n) + 1], shm_data[6 * (joint_n) + 2]] 
            status= [shm_data[joint_n * 6 + 5] for joint_n in range(len(shm_data) // 6)]

 

        # print(f"Joint {joint_n} - t: {t:.2f}, theta_negative: {theta_negative:.2f}, pos_negative: {pos_negative}, pos_positive: {pos_positive}, v_negative: {v_negative}, v_positive: {v_positive}")


        # write data to txt
        with open('joint_data' + str(joint_n) + '.txt', 'a') as f:
            if t == 0:
                f.write(f"Joint {joint_n} : ({plt_x[joint_n-1]:.2f}, {plt_y[joint_n-1]:.2f})\n")
                f.write(f"status: {status}\n\n")
            elif t > 0:
                f.write(f"Joint {joint_n} : dL_self: {dL_self:.3f}, angle: {angle:.2f}\n")
        ...
        shm_data[joint_n*6 + 0] = angle
        shm_data[joint_n*6 + 1] = x
        shm_data[joint_n*6 + 2] = y
        shm_data[joint_n*6 + 3] = vx
        shm_data[joint_n*6 + 4] = vy
        shm_data[joint_n*6 + 5] = status[joint_n]
...
def joint_main_process(queue, args, joint_id):
    # print(f"joint_id: {joint_id}, type(args): {type(args)}")
    list_graph, give_parameters = joint_main(*args)
    queue.put((joint_id, list_graph, give_parameters))


def run(file_path, m, L, g, k, d, dt, joint_num, goal_pos):
    # Set parameters
   ...
   # Set joint parameters
    ...
    for i in range(dis_range): 
        ...
        # Set multiprocessing shared data
        shared_data = np.zeros(joint_num * 7, dtype=np.float64)
        for r in range(joint_num):
            shared_data[r*6 + 0] = theta[r]
            shared_data[r*6 + 1] = plt_x[r]
            shared_data[r*6 + 2] = plt_y[r]
            shared_data[r*6 + 3] = 0.0  # vx
            shared_data[r*6 + 4] = 0.0  # vy
            shared_data[r*6 + 5] = status[r]

        # Set EE position in shared data
        shared_data[joint_num*6 + 1] = EE_x  # EE_x
        shared_data[joint_num*6 + 2] = EE_y  # EE_y

        # print("shared_data", shared_data)
        # time.sleep(100)

        # Create a shared array
        shm = shared_memory.SharedMemory(create=True, size=shared_data.nbytes)
        shm_array = np.ndarray(shared_data.shape, dtype=shared_data.dtype, buffer=shm.buf)
        shm_array[:] = shared_data[:]

        # Check shared memory
        # print("Shared memory created with name:", shm.name)
        # print("Shared memory array:", shm_array)

        args = (
            i, tmax, dt, L, theta, plt_x, plt_y, target_pos_x, target_pos_y,
            joint_num, ran, k, d, shm.name, shm_array
        )
        args_list = list(args)
        with Pool(processes=cpu_count()-3) as p:
            inputs = [(queue, args_list, j) for j in range(joint_num)]
            print(f"joint_num: {joint_num}:start")
            results = p.starmap(joint_main_process, inputs)
