...
def joint_processor(theta_negative,pos_negative,pos_positive,v_negative,L,theta_self,re,joint_n, k, d,v_positive,dis_range, fail_angle, fail_count,count,EE_pos):
    # Set parameters
    pos_self = [re[0], re[1]]
    v_self = [re[2], re[3]]
    ...
    if joint_n == 7:
        dL_data = np.sqrt((EE_pos[0]-pos_self[0])**2 + (EE_pos[1]-pos_self[1])**2)
            # dL_data_pos も同様に処理
        if dL_data < 1e-9:
            P_R_pos = 0.0
            P_R_neg = 0.0
        else:
            P_R_pos = KD* (1 / dL_data)
            P_R_neg = 0
    else:
        dL_data_neg = np.sqrt((v_self[0]+v_negative[0])**2 + (v_self[1]+v_negative[1])**2)
        dL_data_pos = np.sqrt((v_self[0]+v_positive[0])**2 + (v_self[1]+v_positive[1])**2)

        # dL_data_neg が0に近いかどうかで処理を分岐
        if dL_data_neg < 1e-9:  # 非常に小さい値より小さい場合は、力が働かないとする
            P_R_neg = 0.0
        else:
            P_R_neg = KD * (1 / dL_data_neg)

        # dL_data_pos も同様に処理
        if dL_data_pos < 1e-9:
            P_R_pos = 0.0
        else:
            P_R_pos = KD * (1 / dL_data_pos)
    ...
    # ax = fx - fx_negative + d*v_positive[0] - d*v_self[0] - d*v_self[0] + d*v_negative[0] + P_R_neg*cos_angle - P_R_pos*cos_angle
    # ay = fy - fy_negative + d*v_positive[1] - d*v_self[1] - d*v_self[1] + d*v_negative[1] - P_R_pos*sin_angle + P_R_neg*sin_angle
    ...
    # 状態 [x, y, vx, vy] の微分 [vx, vy, ax, ay] をNumPy配列で返す
    return np.array([v_self[0], v_self[1], ax, ay]), dL_self, angle


def runge_kutta(theta_negative, pos_negative, pos_positive, v_negative, v_positive, L, theta_self, re, joint_n , k, d, dt, dis_range, fail_angle, fail_count,count,EE_pos):
    #calculate runge kutta
    # print("runge_kutta")
    k1, dL_self, angle = joint_processor(theta_negative,pos_negative,pos_positive,v_negative,L,theta_self,re,joint_n,k, d, v_positive,dis_range, fail_angle, fail_count,count,EE_pos)
    k2, dL_self, angle = joint_processor(theta_negative,pos_negative,pos_positive,v_negative,L,theta_self, re + k1*dt/2, joint_n, k, d, v_positive,dis_range, fail_angle, fail_count,count,EE_pos)
    k3, dL_self, angle = joint_processor(theta_negative,pos_negative,pos_positive,v_negative,L,theta_self, re + k2*dt/2, joint_n, k, d, v_positive,dis_range, fail_angle, fail_count,count,EE_pos)
    k4, dL_self, angle = joint_processor(theta_negative,pos_negative,pos_positive,v_negative,L,theta_self, re + k3*dt, joint_n ,k , d, v_positive,dis_range, fail_angle, fail_count,count,EE_pos)
    return re + dt/6*(k1 + 2*k2 + 2*k3 + k4), dL_self, angle

def joint_main(i,tmax, dt, L, theta, plt_x, plt_y, target_pos_x, target_pos_y, joint_n, ran, k, d,status, pipe_left, pipe_right, pipes, joint_num, dis_range, fail_angle, fail_count,EE_pos):
    # Set parameters
    t = 0 
    re_x_list = []  
    re_y_list = []  
    ...
    while t < tmax:
        ...
        re, dL_self, angle = runge_kutta(theta_negative, pos_negative, pos_positive, v_negative, v_positive, 
                                         L, theta_self, re, joint_n, k, d, dt, dis_range, fail_angle, fail_count,count,EE_pos)
...
def run(file_path, m, L, g, k, b, dt, joint_num, goal_pos):
   ...
   # Set joint parameters
    ...
    for i in range(dis_range): 
        ...
        for p in range(joint_num):
            ...
            args = (
                ...
                status, pipe_left, pipe_right, pipes, joint_num, dis_range, fail_angle, fail_count, EE_pos
            )
        ...
        # time_list = [results[joint_id]['give_parameters'][6] for joint_id in sorted(results.keys())]
        ...
        #     # 最初の関節(j=0)には先行する関節がないため、dLは0.0とする
        #     current_dl = dL_list[j - 1] if j > 0 else 0.0

        #     row_data = [
        #         i + 1,                     # 現在のdis_rangeステップ番号
        #         j + 1,                     # 関節番号
        #         joint_positions[j][0],     # X座標
        #         joint_positions[j][1],     # Y座標
        #         angles[j],                 # 角度
        #         current_dl                 # dL を追加
        #     ]
        ...
        if judge_success == joint_num-1:
            print(f"dis:{i} -> All joints are successful.")
        # else:
        #     print("\n************************************************************")
    ...
    success = 1  # Set success to 1 if all joints are successful
    return success


    # final_csv_path = os.path.join(file_path, 'simulation_history.csv')
    # print(f"\nWriting all simulation data to {final_csv_path}...")
    
    # # データを書き出す前に、リストにデータが溜まっているか確認すると確実です
    # print(f"Total data rows to write: {len(all_simulation_data)}")
...
if __name__ == '__main__':
    ...
    #記録
    with open(os.path.join(dir_path, "simulation_results.txt"), 'w') as f:
        f.write("start\n")

    #################### run simulation ####################
    print("run simulation")
    for trial in range(num_trials):
        # goal_pos = generate_random_goal(L, joint_num)  # ランダムなゴール座標を生成
        # goal_pos = [101,101]
        print(f"\nTrial {trial + 1}/{num_trials} - Goal Position: {goal_pos}")
        success = run(dir_path, m, L, g, k, d, dt, joint_num, goal_pos)

        if success == 1:
            success_count += 1
            success_positions.append(goal_pos)
        elif success == 0: # これが全ての失敗ケースを処理します
            failure_positions.append(goal_pos)

        goal_str = f"Goal Position: {goal_pos}\n"
        result_str = f"Result: {success}\n"
        print(f"Trial {trial + 1}/{num_trials} - {goal_str}->{result_str}")
        #記録
        with open(os.path.join(dir_path, f"trial_{trial + 1}.txt"), 'a') as f:
            f.write(goal_str)
            f.write(result_str)

    success_rate = (success_count / num_trials) * 100
    print(f"\nTotal Trials: {num_trials}, Success Count: {success_count}, Success Rate: {success_rate:.2f}%")
