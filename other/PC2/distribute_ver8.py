...
#手先位置の更新
def new_pos_eff(first_pos_x, first_pos_y, goal_x, goal_y, end_eff_x, end_eff_y, n, dis_range):
    dt = 0.05
    ...
    for i in range(dis_range):
        # 係数は i+1 を使い、徐々に近づくステップを作成
        step = dt / ((n + i + 1) / 20)  # n に i を足して徐々に減速も調整可能
        
        # 1ステップ分の更新
        current_x = current_x + (goal_x - current_x) * step
        current_y = current_y + (goal_y - current_y) * step
        
        new_pos_x[i] = current_x
        new_pos_y[i] = current_y
...
def joint_main(i,tmax, dt, L, theta, plt_x, plt_y, target_pos_x, target_pos_y, joint_n, ran, k, d,status, pipe_left, pipe_right, pipes, joint_num):
    ...
    try:
        ...
        while t < tmax:
            ...
            if pipe_left is not None:
                if pipe_left.poll(timeout=dt/10):
                    try:
                        ...
                        # print("EOFError: Pipe to left joint is closed.")
                        pass
                else:
            ...
            if pipe_right is not None:
                if pipe_right.poll(timeout=dt/10):
                    try:
                        ...
                        # print("EOFError: Pipe to right joint is closed.")
                        pass
                else:
            ...
            if dL_success == 2:
                print("Joint", joint_n + 1, "is successful")
                break

            if dL_self > ran*1000:
                print("Joint", joint_n + 1, "is failed")
                break
        ...
        with open(f'joint_error_{joint_n}.log', 'w') as f:
            f.write(f"[Joint {joint_n}] Error: {str(e)}\n")
            import traceback
...
def run(file_path, m, L, g, k, b, dt, joint_num, goal_pos):
   ...
   # Set joint parameters
    ...
    # target_pos_x, target_pos_y = new_pos_eff(plt_x, plt_y, goal_pos[0], goal_pos[1], EE_x, EE_y,joint_num, dis_range)
    ...
    for i in range(dis_range): 
        ...
        for p in range(joint_num):
            ...
            args = (
                ...
                target_pos_x, target_pos_y, joint_n, spring_range, k, b,
