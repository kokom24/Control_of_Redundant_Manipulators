...
def f_n(L, K, D, n, theta_0, x_vec,end_eff_x,end_eff_y,judge,t, fail_count, angle,count):
    ...
    for i in range(n):
        ...
        # if dL[i] <= 0:
        #     dL[i] = 1.0*10**-5  # dLが負の値にならないようにする
            # print("dLが負の値になりました。dLを1.0*10**-5に設定しました。")
    # ##print("dL:",dL)
...
#nリンクの時のグラフの更新
def new_posi_spring_n(tmax, dt, t, x_vec, theta_0, gole_position,n, L, K, D, initial_position,first_pos_x,first_pos_y, 
                      spring_len_judg, theta_n, ran,broken_joint,break_status,file_dL, file_angle, file_path):
    # グラフの準備
    fig, ax = plt.subplots()
    plt_x = np.zeros(n+1)
    plt_y = np.zeros(n+1)
    ...
    while t < tmax:
        ...
        if judge == 1:
            ...
            # derivative_error_x = error_x / dt
            # derivative_error_y = error_y / dt
            ...
            # delta_K = np.clip(delta_K, -50, 50)
            # delta_D = np.clip(delta_D, -10, 10)
            ...
            iterm_x += error_x * dt  
            iterm_y += error_y * dt
            iterm_x = np.clip(iterm_x, -20, 20)  
            iterm_y = np.clip(iterm_y, -20, 20)

            dterm_x = (delta_error_x / dt) if dt > 0 else 0
            dterm_y = (delta_error_y / dt) if dt > 0 else 0
            dterm_x = np.clip(dterm_x, -2, 2)
            dterm_y = np.clip(dterm_y, -2, 2)

            prev_error_x = error_x
            prev_error_y = error_y

            delta_k = (pterm_x+ pterm_y)/2 + Ki * (iterm_x + iterm_y) /2+ Kd * (dterm_x + dterm_y)/2
            delta_d = (dterm_x + dterm_y) / 2
            delta_k = np.clip(delta_k, -50, 50) 
            delta_d = np.clip(delta_d, -10, 10)
            K = np.clip(K + delta_k, 0, 200)
            D = np.clip(D + delta_d, 0.0, 20)
        ...
        # csvファイルに書き込み
        # with open(file_dL, 'a', newline='') as f:
        #     writer = csv.writer(f)
        #     writer.writerow([t] + dL.tolist())
        # with open(file_angle, 'a', newline='') as f:
        #     writer = csv.writer(f)
        #     writer.writerow([t] + angle.tolist())

        # # K,Dの値を書き込み
        # file_gain = "gain.csv"
        # with open(file_gain, 'a', newline='') as f:
        #     writer = csv.writer(f)
        #     # writer.writerow([t, K, D,error_x, error_y, integral_error_x, integral_error_y, derivative_error_x, derivative_error_y,delta_K, delta_D])
        #     writer.writerow([t, K, D, pterm_x, pterm_y, iterm_x, iterm_y, dterm_x, dterm_y, delta_k, delta_d])
...
def run(broken_joint, break_status):
    #パラメータ設定
    n = 10 #リンク数
    L = 10 #リンクの長さ
    ran = 1 #目標位置の許容範囲
    ...
    with open(file_angle, mode='w') as f:
        f.write("")
        # 条件を書き込む
        f.write(f"n: {n}, L: {L}, ran: {ran}, theta_0: {theta_0}, theta_n: {theta_n}, initial_position: {initial_position}, spring_len_judge: {spring_len_judge}\n")
    ...
    for i in tqdm(range(loop), desc="Simulation Progress"):
        ...
        success = new_posi_spring_n(tmax, dt, t, x_vec_n, theta_0, gole_position,n, L, k_pa, D_pa, initial_position,
                                    first_pos_x,first_pos_y, spring_len_judge, theta_n, ran, broken_joint,break_status,file_dL, file_angle, file_path)
