...
def run(file_path, m, L, g, k, b, dt, joint_num, goal_pos):
   ...
   # Set joint parameters
    ...
    for i in range(dis_range): 
        ...
        print(f"Simulation for dis_range {i + 1} completed.")
    
    success = 1  # If the loop completes without breaking, set success to 1
    return success


def run_simulation(dir_path, m, L, g, k, d, dt, joint_num, goal_pos):
    #パラメータ設定
    success = 0 #成功判定
    test = 0 #テスト回数
    all_success = 0 #成功数
    ...
    file_path = os.path.join(dir_path, "success_rate.txt")
    ...
    for i in range(loop):
        ...
        while goal_position == initial_position or np.sqrt((goal_position[0] - initial_position[0])**2 + (goal_position[1] - initial_position[1])**2) > L * n:
            goal_position = [np.random.randint(0,100), np.random.randint(0, 100)]
        ...
        # シミュレーション
        success = run(dir_path, m, L, g, k, d, dt, joint_num, goal_position)
        print("success:", success)

        # 成功数を計算
        if success == 1:
            all_success += 1
            success_positions.append(goal_position)
        elif success == 0:
            failure_positions.append(goal_position)
        elif success == None:
            failure_positions.append(goal_position)
        elif success == "cross":
            failure_positions.append(goal_position)

        print("Goal Position:", goal_position, "Success:", success)

        #成功を記録(ファイルパスに随時書き込み)
        with open(file_path, 'a') as file:
            file.write(f"Goal Position: {goal_position}, Success: {success}\n")
        print("write success")
...
def setup(dir_path, m, L, g, k, d, dt, joint_num, goal_pos):
    ...
    #write file
    porpose_write = f"target position: {goal_pos}\n"
    ...
    # Run simulation
    run_simulation(dir_path, m, L, g, k, d, dt, joint_num, goal_pos)
...
if __name__ == '__main__':
    ...
    setup(dir_path, m, L, g, k, d, dt, joint_num, goal_pos)
