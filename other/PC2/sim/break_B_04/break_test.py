...
if __name__ == '__main__':
    ...
    parameters = [(i, j, k, l) for i in range(10) for j in range(10) for k in range(10) for l in range(10)
                  if len(set([i, j, k, l])) == 4]

    # Specify the number of processes
    num_processes = 10  # Adjust this number based on your machine's capabilities

    # Use multiprocessing Pool to parallelize the computation
    with Pool(processes=num_processes) as pool:
        # Use tqdm to display progress
        results = list(pool.imap(process_joint, parameters))
    ...
    for success_count, try_count, broken_joint, indices in results:
        ...
        # Check if the combination is 2-adjacent (difference of 1)
        if len(broken_joint) == 4:
            adjacents = [abs(indices[x] - indices[x+1]) for x in range(3)]
            ...
            # Check if the combination is 3-adjacent (all differences of 1)
            ...
            # Check if the combination is 4-adjacent (all differences of 1)
    ...
    print(f"2-adjacent success_rate: {adjacent_success_rate * 100}%")
    print(f"3-adjacent success_rate: {three_adjacent_success_rate * 100}%")
    print(f"4-adjacent success_rate: {four_adjacent_success_rate * 100}%")
    print("-----------------------------finish-----------------------------")