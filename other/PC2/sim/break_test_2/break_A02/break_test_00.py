...
def runge_kutta_n(L, K, D, n, theta_0, x_vec, dt,end_eff_x,end_eff_y, judge, t, fail_count, angle, count):
    ...
    if judge == 1 and t > fail_time:
        fail_set_angle = np.zeros(1)
        fail_set_angle[0] = np.deg2rad(np.rad2deg(angle[2]) * 0.8)
        # fail_angle[1] = angle[3] *0.9
        # fail_angle[2] = angle[4] * 0.9
        fail_count = 1
    else:
        fail_angle = np.zeros(1)

    if fail_count == 1:
        print("re_angle[2]:",angle[2])
        fail_set_angle[0] = np.deg2rad(np.rad2deg(angle[2]) * 0.8)
        # set fail angle
        for i in range(n-1):
            fail_angle = angle[i]
            fail_angle[2] = fail_set_angle[0]
        print("after_angle[2]:",angle[2])
    else:
        pass

    return x_vec + (dt / 6) * (k1 + 2 * k2 + 2 * k3 + k4) , dL, angle, fail_count, fail_angle
...
def new_posi_spring_n(tmax, dt, t, x_vec, theta_0, gole_position,n, L, K, D, initial_position,first_pos_x,first_pos_y, spring_len_judg,ran,theta_n):
    ...
    with open(csv_break_file_path, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Time', 'Angle1', 'Angle2', 'Angle3', 'Angle4', 'Angle5', 'Angle6', 'Angle7', 'Angle8', 'Angle9', 'Angle10'])
    ...
    while t < tmax:
        ...
        #アニメーションの初期化
        x_vec, dL, angle, fail_count, fail_angle = runge_kutta_n(L, K, D, n, theta_0, x_vec, dt,plt_x,plt_y, judge, t, fail_count,angle, count)
        # 各要素の更新
        x = x_vec[0:n-1]
        y = x_vec[(n-1):2*(n-1)]
        ...
        with open(csv_file_path, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            wt_angle = np.rad2deg(angle)
            writer.writerow([t, wt_angle[0], wt_angle[1], wt_angle[2], wt_angle[3], wt_angle[4], wt_angle[5], wt_angle[6], wt_angle[7], wt_angle[8], wt_angle[9]])

        with open(csv_break_file_path, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            wt_fail_angle = np.rad2deg(fail_angle)
            writer.writerow([t, wt_fail_angle[0], wt_fail_angle[1], wt_fail_angle[2], wt_fail_angle[3], wt_fail_angle[4], wt_fail_angle[5], wt_fail_angle[6], wt_fail_angle[7], wt_fail_angle[8], wt_fail_angle[9]])
...
if __name__ == '__main__':
    ...
    csv_break_file_path = os.path.join(dir_path, "break_angle.csv")
