...
def f_n(L, K, D, n, theta_0, x_vec,end_eff_x,end_eff_y,judge,t, fail_count, angle,count):
    ...
    if judge == 1 and t > fail_time:
        fail_angle = np.zeros(4)
        fail_angle[0] = angle[1] *0.8
        fail_angle[1] = angle[2]*0.8
        fail_angle[2] = angle[4]*0.8
        fail_angle[3] = angle[3]*0.8
        fail_count = 1
    else:
        fail_angle = np.zeros(4)
