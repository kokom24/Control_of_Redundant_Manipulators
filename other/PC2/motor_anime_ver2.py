...
def f_n(L, K, D, n, theta_0, x_vec,end_eff_x,end_eff_y,judge,t, fail_count, angle):
    ...
    fail_angle[1] = angle[4]
    fail_angle[2] = angle[6]
...
def new_posi_spring_n(tmax, dt, t, x_vec, theta_0, gole_position,n, L, K, D, initial_position,first_pos_x,first_pos_y, spring_len_judg):
    ...
    # ===== 各リンク角度 theta_θ =====
    theta_theta = np.zeros(n)
    for i in range(n):
        dx = x[i+1] - x[i]
        dy = y[i+1] - y[i]
        theta_theta[i] = math.atan2(dy, dx)
    #リンクの位置を計算
    for i in range(n):
        plt_x[i+1] = plt_x[i] + L * math.cos(theta_theta[i])
        plt_y[i+1] = plt_y[i] + L * math.sin(theta_theta[i])
    initial_angle = theta_theta
    angle = np.zeros(n)
    angle = initial_angle.copy()
    print(f"initial_angle(deg):{np.rad2deg(initial_angle)}")
    # time.sleep(100)
    ...
    ax.set_xlim(-L *(n * 1.1),L *(n * 2.1))  # x軸の範囲を設定
    ax.set_ylim(-L *(n * 1.1),L *(n * 2.1))  # y軸の範囲を設定
    ax.grid(True)  # グリッドを表示
    plt.connect('key_press_event', on_key)
    #グラフの左上に”spoe”と文字を表示
    plt.text(-L *(n * 1.1), L *(n * 2.1)+2, "start", fontsize=20, color="red")
    # #print("plt_x:",plt_x)
    # #print("plt_y:",plt_y)
    # plt.show()
    # time.sleep(100)
    ...
    while t < tmax:
        ...
        # グラフの初期化
        ax.clear()
        ax.set_xlim(-L *(n * 1.1),L *(n * 1.1))  # x軸の範囲を設定
        ax.set_ylim(-L *(n * 1.1),L *(n * 1.1))  # y軸の範囲を設定
...
# 初期条件の計算（sin波ベース）
def first_position(n, L, initial_position):
    """
    n : リンク数
    L : リンク長（※今回は sin 波の点を優先）
    ...
    print("theta_θ (deg):", np.rad2deg(theta_theta))
...
if __name__ == '__main__':
    ...
    first_pos_x, first_pos_y = first_position(n, L, initial_position)
