...
def joint_main(i,tmax, dt, L, theta, plt_x, plt_y, target_pos_x, target_pos_y, joint_n, ran, k, d,status, pipe_left, pipe_right, pipes, joint_num, is_failure, fail_angles_map):
    ...
    # print("plt_x", plt_x)
    # print("plt_y", plt_y)
    # print("pos_positive(joint_n)", pos_positive, joint_n)
    v_negative = [0, 0]
...
def run(m, L, g, k, b, dt, joint_num, goal_pos, file_path):
    ...
    ax.grid(True)
    re = np.zeros(4)
    fail_angle = np.zeros(joint_num)
    mani_x = np.zeros(joint_num)
    mani_y = np.zeros(joint_num)
    normal_angle = np.zeros(joint_num)
    final_x = np.zeros(joint_num)
    final_y = np.zeros(joint_num)
    z =0
    initial_position = np.zeros((joint_num, 2))
   ...
   # Set joint parameters
    ...
    # ax.plot(goal_pos[0],goal_pos[1],color="red",marker="o")
    #グラフの設定
    # ax.set_xlim(-L *(joint_num * 0.1),L *(joint_num * 1.1))  # x軸の範囲を設定
    # ax.set_ylim(-L *(joint_num * 0.1),L *(joint_num * 1.1))  # y軸の範囲を設定
    ...
    while t < tmax:
        ...
        if not is_failure and file_count > 0:
            ...
            if report_fail == True:
                ...
                with open(conditions_file_path, 'a', encoding='utf-8') as f:
        ...
        #関節の故障を計算   
        for i in range(joint_num):
            fail_angle[i] = angles[i]
            normal_angle[i] = angles[i]
            if i in fail_angles_map:
                fail_angle[i] = fail_angles_map[i]
        ...
        for i in range(joint_num):
            if i == 0:
                ...
                # position[i][0] = mani_x[i]
                # position[i][1] = mani_y[i]
            elif i == joint_num-1:
        ...
        # リンクの更新
        mani_plt_x = np.concatenate([initial_position[0:1,0], mani_x])
        mani_plt_y = np.concatenate([initial_position[0:1,1], mani_y])
        final_plt_x = np.concatenate([initial_position[0:1,0], final_x])
        final_plt_y = np.concatenate([initial_position[0:1,1], final_y])
        # print("mani_plt_x:", mani_plt_x)
        # print("mani_plt_y:", mani_plt_y)
        # print("final_plt_x:", final_plt_x)
        # print("final_plt_y:", final_plt_y)
        ...
        #     plt_x[r] = mani_plt_x[r]
        #     plt_y[r] = mani_plt_y[r]
        # print("plt_x", plt_x)
        ...
        # print(f"time:{t:.2f}s : ゴールまであと{mani_plt_x[joint_num] - goal_pos[0]:.2f} (x), {mani_plt_y[joint_num] - goal_pos[1]:.2f} (y)")
        # print(f"time:{t:.2f}s : ゴールまであと{plt_x[joint_num] - goal_pos[0]:.2f} (x), {plt_y[joint_num] - goal_pos[1]:.2f} (y)\n")
        # print("time:", t)

        if np.all(goal_pos[0] - goal_range <= mani_plt_x[joint_num]) and np.all(mani_plt_x[joint_num] <= goal_pos[0] + goal_range) and np.all(goal_pos[1] - goal_range <= mani_plt_y[joint_num]) and np.all(mani_plt_y[joint_num] <= goal_pos[1] + goal_range):
        # if np.all(goal_pos[0] - goal_range <= plt_x[joint_num]) and np.all(plt_x[joint_num] <= goal_pos[0] + goal_range) and np.all(goal_pos[1] - goal_range <= plt_y[joint_num]) and np.all(plt_y[joint_num] <= goal_pos[1] + goal_range):
            ...
            # ax.clear()
            # plt.text(-L *(joint_num), L *(joint_num * 1.1)+2, "fin", fontsize=20, color="red")                    
            # print("ゴールしました。")
            #リンクの描画
            # for i in range(joint_num):
            #     ax.plot([final_plt_x[i],final_plt_x[i+1]],[final_plt_y[i],final_plt_y[i+1]],color="black",marker="o",linestyle="-")
            #     plt.gca().set_aspect('equal', adjustable='box')
            # ax.set_xlim(-L *(joint_num * 0.1),L *(joint_num * 1.1))  # x軸の範囲を設定
            # ax.set_ylim(-L *(joint_num * 0.1),L *(joint_num * 1.1))  # y軸の範囲を設定
            # plt.draw()
            ...
            return success
        #pltの更新
        end_eff_x, end_eff_y = new_pos_eff(first_x, first_y, goal_pos[0], goal_pos[1], mani_plt_x[joint_num], mani_plt_y[joint_num], joint_num)
        for r in range(joint_num + 1):
            plt_x[r] = mani_plt_x[r]
            plt_y[r] = mani_plt_y[r]
        for i in range(joint_num + 1):
            plt_x[i] = mani_plt_x[i]
            plt_y[i] = mani_plt_y[i]
            if i  ==10:
                plt_x[i] = end_eff_x
                plt_y[i] = end_eff_y
        # print("plt_x", plt_x)
...
if __name__ == '__main__':
    ...
    # ===== 途中再開用：既存結果の読み込み =====
    re_results_file_path = r"C:\Users\rsait\OneDrive - 東京理科大学\デスクトップ\git\M1_styudy\result\summary_results_final_1000_trials.txt"  # 既存の結果ファイルパス
    ...
    for i in tqdm(range(start_trial_index, num_trials),desc="Simulating Trials", unit="trial"):
        ...
        while (random_goal == initial_position or np.sqrt(random_goal[0]**2 + random_goal[1]**2) < 0.85 * L * joint_num or np.sqrt(random_goal[0]**2 + random_goal[1]**2) > L * joint_num):
            random_goal = [np.random.randint(0,100), np.random.randint(0, 100)]
        if first == 1:
            first = 0
        ...
        result_status = run(m, L, g, k, d, dt, joint_num, random_goal, current_trial_dir)
