...
if __name__ == '__main__':
    ...
    parameters = [(i, j, k) for i in range(10) for j in range(10) for k in range(10) if len(set([i, j, k])) == 3]
    ...
    for success_count, try_count, broken_joint, indices in results:
        ...
        if len(broken_joint) == 3:
            adjacents = [abs(indices[x] - indices[x+1]) for x in range(2)]
