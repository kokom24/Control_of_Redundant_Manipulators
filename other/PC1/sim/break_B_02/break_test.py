...
def process_joint(params):
    ...
    return success_count, try_count, broken_joint, i, j
...
if __name__ == '__main__':
    ...
    parameters = [(i, j) for i in range(10) for j in range(10)]
    ...
    for success_count, try_count, broken_joint, i, j in results:
        ...
        # Check if the combination is adjacent (difference of 1)
        if abs(i - j) == 1:
    ...
    # Write the adjacent results and success rate to a separate file
    adjacent_success_rate = adjacent_success_count / adjacent_try_count if adjacent_try_count > 0 else 0
...
#     for i in range(10):
#             for j in range(10):
#                 # Set the parameters and ensure the combination is unique
#                 broken_joint = tuple(sorted([i, j]))
...
#                 # Write to the file in the newly created directory
#                 porpose_write = f"{len(broken_joint)} angle break.\n angle {broken_joint} is fixed\n\n"
#                 with open("result.txt", mode='a') as f:
#                     f.write(porpose_write)
...
#                     adjacent_success_count += success_count
#                     adjacent_try_count += try_count
#                     adjacent_success_rate = adjacent_success_count / adjacent_try_count
#                     adjacent_results.append(f"broken_joint: {broken_joint}, success_rate: {adjacent_success_rate * 100}%\n")

#                 print("-----------------------------finish-----------------------------")
    
#     # Write the success rate to the file
#     success_rate_write = f"total success_rate: {success_rate * 100}%\n"
#     with open("result.txt", mode='a') as f:
#         f.write(success_rate_write)
