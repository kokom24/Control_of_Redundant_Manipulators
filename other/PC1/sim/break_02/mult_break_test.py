...
def f_n(L, K, D, n, theta_0, x_vec,end_eff_x,end_eff_y,judge,t, fail_count, angle):
    ...
    R = np.zeros(n-1)
    for i in range(n-1):
        R[i] = math.sqrt(2 * LR**2 *(1-math.cos(angle[i])) )
...
def simulate(params):
    n, L, ran, theta_0, theta_n, initial_position, spring_len_judge, tmax, dt, t, loop,K_pa, D_pa = params
    ...
    success_rate = all_success / loop

    return success_positions, failure_positions, timeover_positions, cross_positions, success_rate
...
def run():
    ...
    print("cpu_count:", num_processes)
    params = (n, L, ran, theta_0, theta_n, initial_position, spring_len_judge, tmax, dt, t, loop, K_pa, D_pa)

    with Pool(processes=15) as pool:
        results = list(tqdm(pool.imap(simulate, [params] * num_processes), total=num_processes))
    print(results)

    # 結果の統合
    all_success_positions = []
    all_failure_positions = []
    all_timeover_positions = []
    all_cross_positions = []
    total_success_rate = 0

    for result in results:
        success_positions, failure_positions, timeover_positions, cross_positions, success_rate = result
        all_success_positions.extend(success_positions)
        all_failure_positions.extend(failure_positions)
        all_timeover_positions.extend(timeover_positions)
        all_cross_positions.extend(cross_positions)
        total_success_rate += success_rate

    # 成功率の平均を計算
    total_success_rate /= num_processes
    ...
    plt.scatter([pos[0] for pos in all_success_positions], [pos[1] for pos in all_success_positions], color='blue', label='Success', marker="x")
    plt.scatter([pos[0] for pos in all_failure_positions], [pos[1] for pos in all_failure_positions], color='red', label='Failure', marker="x")
    plt.scatter([pos[0] for pos in all_timeover_positions], [pos[1] for pos in all_timeover_positions], color='green', label='Timeover', marker="x")
    plt.scatter([pos[0] for pos in all_cross_positions], [pos[1] for pos in all_cross_positions], color='yellow', label='Cross', marker="x")
    plt.xlabel('X-axis')
    plt.ylabel('Y-axis')
    ...
    para_to_write = f"n: {n}\nL: {L}\nloop: {loop}\n"
    data_to_write = f"theta_0: {theta_0}\ntheta_n: {theta_n}\nsuccess_rate: {total_success_rate * 100}%\n"
