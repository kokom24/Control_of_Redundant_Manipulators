...
# dL_negativeの計算... (以下に続く)
    ...
    # 反発力に関するパラメータ
    k_repulsive = 0.01  # 反発係数。値が大きいほど強く反発する。
...
def joint_main(i,tmax, dt, L, theta, plt_x, plt_y, target_pos_x, target_pos_y, joint_n, ran, k, d,status, pipe_left, pipe_right, pipes, joint_num, is_failure, fail_angles_map):
    ...
    while t < tmax:
        ...
        if try_count > 50:
            ...
            condition_B_met = (consecutive_dL_count >= 50)
   ...
   # Set joint parameters
    ...
    while t < tmax:
        ...
        if not is_failure and file_count > 0:
            ...
            # 固定する関節番号(2,3,4)とその角度で辞書を作成
            fail_angles_map = {
                # 3: angles[3], # joint_n=2 の角度
                4: angles[4], # joint_n=3 の角度
                5: angles[5], # joint_n=4 の角度
                # 6: angles[6], # joint_n=4 の角度
            }
            
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print(f"FAILURE TRIGGERED: Joints 2,4,6,8 are now fixed.")
            print(f"Fixed angles map (rad): {fail_angles_map}")
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
