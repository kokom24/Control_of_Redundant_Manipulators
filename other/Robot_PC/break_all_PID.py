...
#nリンクの時のグラフの更新
def new_posi_spring_n(tmax, dt, t, x_vec, theta_0, gole_position,n, L, K, D, initial_position,first_pos_x,first_pos_y, spring_len_judg, theta_n, ran,broken_joint,break_status):
    # グラフの準備
    fig, ax = plt.subplots()
    plt_x = np.zeros(n+1)
    plt_y = np.zeros(n+1)
...
def run(broken_joint, break_status):
    ...
    for i in tqdm(range(loop), desc="Simulation Progress"):
        ...
        # シミュレーション
        success = new_posi_spring_n(tmax, dt, t, x_vec_n, theta_0, gole_position,n, L, k_pa, D_pa, initial_position,first_pos_x,first_pos_y, spring_len_judge, theta_n, ran, broken_joint,break_status)
