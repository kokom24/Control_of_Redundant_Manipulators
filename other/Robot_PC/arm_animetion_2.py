...
def new_posi_spring_n(tmax, dt, t, x_vec, theta_0, gole_position,n, L, K, D, initial_position,first_pos_x,first_pos_y, spring_len_judg,ran,theta_n):
    ...
    while t < tmax:
        ...
        if judge == 1:
            ...
            plt.draw()
            plt.pause(dt)  # グラフの更新を表示するための待機時間を設定
        ...
        #     writer = csv.writer(csvfile)
        #     writer.writerow([t, angle[0], angle[1], angle[2], angle[3], angle[4], angle[5], angle[6], angle[7], angle[8], angle[9]])
...
if __name__ == '__main__':
    ...
    #シミュレーション
    new_posi_spring_n(tmax, dt, t, x_vec_n, theta_0, gole_position,n, L, k_pa, D_pa, initial_position,first_pos_x,first_pos_y, spring_len_judge,ran,theta_n)